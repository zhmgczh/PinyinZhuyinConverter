...
class Converter:
    ...
    def extract_all_pinyin(self, article: str) -> list:
        ...
        while i < len(article):
            if article[i] in self.pinyin_characters:
                ...
                while (
                    index < len(article)
                    and article[index] in self.pinyin_characters
                    or "'" == article[index]
                    or " " == article[index]
                ):
                    index += 1
    ...
    def extract_all_zhuyin(self, article: str) -> list:
        ...
        while i < len(article):
            if article[i] in self.zhuyin_characters:
                ...
                while (
                    index < len(article)
                    and article[index] in self.zhuyin_characters
                    or " " == article[index]
                ):
                    index += 1
                particle = article[i:index].strip()
            ...
            if "" != pinyin_list[i - 1] and "" != pinyin_list[i]:
                ...
                if not need_split and 1 < len(comb):
                    for j in range(1, len(comb)):
                        if (
                            j != len(pinyin_list[i - 1])
                            and comb[:j] in self.pinyin_syllables
                            and comb[j:] in self.pinyin_syllables
                        ):
                            need_split = True
                            break
    ...
    def zhuyin_list_to_string(self, zhuyin_list: list, zhuyin_split: str = " ") -> str:
        ...
        for i in range(1, len(zhuyin_list)):
            if (
                "" != zhuyin_list[i - 1]
                and "" != zhuyin_list[i]
                and zhuyin_list[i][0] not in self.zhuyin_tones
                and zhuyin_list[i - 1][-1] not in self.zhuyin_tones
            ):
                ...
                if not need_split and 1 < len(comb):
                    for j in range(1, len(comb)):
                        if (
                            j != len(previous_zhuyin)
                            and comb[:j] in self.zhuyin_syllables
                            and comb[j:] in self.zhuyin_syllables
                        ):
                            need_split = True
                            break
    ...
    def add_tone_to_pinyin_syllable(self, pinyin: str, tone: int):
        ...
        for i in range(len(pinyin)):
            if (
                not added
                and not skipped
                and 1 < len(vowel_indices)
                and pinyin[i] in ("i", "u", "ü")
            ):
    ...
    def convert_pinyin(
        self, pinyin: str, pinyin_split="'", zhuyin_split: str = " "
    ) -> tuple:
                ...
                for pure_syllable in syllables[i]:
                    ...
                    for character in pinyin_syllable:
                        if character in self.back_vowels:
                            new_tone = self.vowels[self.back_vowels[character]].index(
                                character
                            )
                            if new_tone < tone:
                                tone = new_tone
                    pinyin_list.append(
                        self.add_tone_to_pinyin_syllable(pure_syllable, tone)
                    )
                    zhuyin_list.append(
                        self.add_tone_to_zhuyin_syllable(
                            self.pinyin_syllables[pure_syllable], tone
                        )
                    )
                    pinyin = pinyin[len(pure_syllable) :]
        return (
            self.pinyin_list_to_string(pinyin_list, pinyin_split),
            self.zhuyin_list_to_string(zhuyin_list, zhuyin_split),
            pinyin_list,
            zhuyin_list,
        )

    def convert_zhuyin(
        self, zhuyin: str, pinyin_split="'", zhuyin_split: str = " ", irregular=False
    ) -> tuple:
        ...
        for i in range(len(syllables)):
            if None != syllables[i]:
                for pure_syllable in syllables[i]:
                    ...
                    if irregular:
                        if (
                            right_index + 1 < len(zhuyin)
                            and zhuyin[right_index + 1] in self.zhuyin_tones
                        ):
                            tone = self.zhuyin_tones.index(zhuyin[right_index + 1])
                    else:
                        if (
                            left_index > 0
                            and zhuyin[left_index - 1] == self.zhuyin_tones[4]
                        ):
                            tone = 4
                        elif (
                            right_index + 1 < len(zhuyin)
                            and zhuyin[right_index + 1] in self.zhuyin_tones[:4]
                        ):
                            tone = self.zhuyin_tones.index(zhuyin[right_index + 1])
                    pinyin_list.append(
                        self.add_tone_to_pinyin_syllable(
                            self.zhuyin_syllables[pure_syllable], tone
                        )
                    )
                    zhuyin_list.append(
                        self.add_tone_to_zhuyin_syllable(pure_syllable, tone)
                    )
...
def main():
    ...
    print(
        converter.convert_zhuyin(
            "ㄔㄨㄢㄕㄤˋㄌㄩˋㄆㄧˊㄠˇㄌㄧㄡˊㄌㄧˋㄉㄜ˙ㄔㄤˋㄕㄢㄍㄜ", irregular=True
        )
    )
    ...
    print(
        converter.convert_pinyin(
            "chuānshànglǜpí'ǎoliúlìdechàngshān'gē", pinyin_split="|"
        )
    )
    ...
    print(
        converter.extract_all_pinyin(
            """《大陸居民臺灣正體字講義》一簡多繁辨析之「裊、嬝、嫋、褭」→「袅」
辨音：「裊、嬝、嫋、褭」音niǎo。
